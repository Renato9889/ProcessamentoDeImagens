...
for i in range(il):
    for j in range(ic):
        nova.itemset((i,j),image.item(i,j))

for i in range(il-1):
    for j in range(ic-1):
        if (i is not 0 or j is not 0):
            if(image.item(i, j - 1) == 0 and image.item(i, j)==255 and image.item(i, j + 1)==0):
                nova.itemset((i, j),0)
            else:
                if( (image.item(i - 1, j) == 0 and image.item(i, j)==255 and image.item(i + 1, j) == 0)):
                    nova.itemset((i, j), 0)



cv2.imshow("Fechamento",nova)
cv2.waitKey(0)
